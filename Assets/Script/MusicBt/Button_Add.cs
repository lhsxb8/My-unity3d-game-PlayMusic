...
public class Button_Add : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.KeypadPlus) && Time.time > nextclick)
        {
            nextclick = Time.time + rateclick;
            if (situation == 0)
            {

                gameObject.GetComponent<AudioSource>().clip = (AudioClip)Resources.Load("钢琴音色WAV/41-C# -小字1组", typeof(AudioClip));
            ...
            if (situation == 2)
            {
                gameObject.GetComponent<AudioSource>().clip = (AudioClip)Resources.Load("钢琴音色WAV/65-C#  -小字3组", typeof(AudioClip));
                gameObject.GetComponent<AudioSource>().Play();
            }
        }
